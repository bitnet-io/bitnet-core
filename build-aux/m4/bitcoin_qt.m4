dnl Copyright (c) 2013-2016 The Bitnet Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set bitnet_enable_qt to false. Else, exit.
AC_DEFUN([BITCOIN_QT_FAIL],[
  if test "$bitnet_qt_want_version" = "auto" && test "$bitnet_qt_force" != "yes"; then
    if test "$bitnet_enable_qt" != "no"; then
      AC_MSG_WARN([$1; bitnet-qt frontend will not be built])
    fi
    bitnet_enable_qt=no
    bitnet_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([BITCOIN_QT_CHECK],[
  if test "$bitnet_enable_qt" != "no" && test "$bitnet_qt_want_version" != "no"; then
    true
    $1
  else
    true
...
AC_DEFUN([BITCOIN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build bitnet-qt GUI (default=auto)])],
    [
     bitnet_qt_want_version=$withval
     if test "$bitnet_qt_want_version" = "yes"; then
       bitnet_qt_force=yes
       bitnet_qt_want_version=auto
     fi
    ],
    [bitnet_qt_want_version=auto])
  ...
  AS_IF([test "$with_gui" = "qt5_debug"],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); bitnet_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl Outputs: bitnet_enable_qt, bitnet_enable_qt_dbus, bitnet_enable_qt_test
AC_DEFUN([BITCOIN_QT_CONFIGURE],[
  ...
  if test "$bitnet_cv_static_qt" = "yes"; then
  ...
  BITCOIN_QT_CHECK([
    bitnet_enable_qt=yes
    bitnet_enable_qt_test=yes
    if test "$have_qt_test" = "no"; then
      bitnet_enable_qt_test=no
    fi
    bitnet_enable_qt_dbus=no
    if test "$use_dbus" != "no" && test "$have_qt_dbus" = "yes"; then
      bitnet_enable_qt_dbus=yes
    fi
  ...
  if test $bitnet_enable_qt = "yes"; then
    AC_MSG_RESULT([$bitnet_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$bitnet_enable_qt])
  fi
...
dnl Output: bitnet_cv_static_qt=yes|no
AC_DEFUN([_BITCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, bitnet_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [bitnet_cv_static_qt=yes],
      [bitnet_cv_static_qt=no])
    ])
])
