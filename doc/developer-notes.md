...
- [Development guidelines](#development-guidelines)
    - [General Bitnet Core](#general-bitnet-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Bitnet Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play bitnets" on the test network, if you
...
Bitnet Core is a multi-threaded application, and deadlocks or other
...
Bitnet Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Bitnet Core, where everyone uses
...
pay attention to for reviewers of Bitnet Core code.
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Bitnet Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
Some of these are maintained by active developers of Bitnet Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Bitnet Core so that they can be integrated
...
file descriptors it will cause problems with Bitnet's `select()` loop, because
...
would be to revert the upstream fix before applying the updates to Bitnet's
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Bitnet Core, the wallet was always
