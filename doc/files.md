...
`./`               | `qtum.conf`        | User-defined [configuration settings](bitcoin-conf.md) for `bitnetd` or `bitnet-qt`. File is not written to by the software and must be created manually. Path can be specified by `-conf` option
