Bitnet Core version 0.13.2 is now available from:

  <https://download.bitnet.org/bitnet-0.13.2.1/>
...
ECDSA signatures inside Bitnet transactions now use validation using
...
Previous versions of Bitnet Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Bitnet Core was to raise the default minimum
relay fee.

Bitnet Core 0.13.2 will have a strict maximum size on the mempool. The
...
Bitnet Core 0.13.2 also introduces new default policy limits on the length and
...
Bitnet Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Bitnet Core will relay transactions with
...
entirely in Bitnet Core 0.13, and it is at this time undecided whether the
...
Bitnet Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
Bitnet Core automatically creates a hidden service to listen on, without
manual configuration. Bitnet Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Bitnet Core is listening, and
...
Bitnetd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Bitnet
Core to use floating fees. By default, Bitnet Core will use floating
fees.
...
At all times, Bitnet Core will cap fees at `-maxtxfee=<x>` (default:
0.10) BIT.
Furthermore, Bitnet Core will never create transactions paying less than
...
Various code modernizations have been done. The Bitnet Core code base has
...
- `bitnet-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `bitnet-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Bitnet scripting system that in combination with
...
Bitnet transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Bitnet Core.
...
The two phases are distinct in the Bitnet-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Bitnet wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Bitnet.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Bitnet block need to be stored by nodes in order to validate future
...
- **Increased security for multisig:** Bitnet addresses (both P2PKH addresses
  ...
  believed to be provided by Bitnet's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Bitnet's current ledger (called the UTXO set),
  ...
  note that Bitnet Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Bitnet users to individually opt-in to almost any change in the Bitnet
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Bitnet Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Bitnet
client to Bitnet Core 0.13.2 to signal for segwit support. When a
...
Since Bitnet Core 0.10.0, nodes have defaulted to only relaying and
