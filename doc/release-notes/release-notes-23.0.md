...
Bitnet Core version 23.0 is now available from:
...
- Before this release, Bitnet Core had a strong preference to try to connect only to peers that listen on port 8333. As a result of that, Bitnet nodes listening on non-standard ports would likely not get any Bitnet Core peers connecting to them. This preference has been removed. (#23542)
...
Bitnet Core release binaries for Linux now include experimental tracepoints which
...
* On startup, the list of banned hosts and networks (via `setban` RPC) in
  `banlist.dat` is ignored and only `banlist.json` is considered. Bitnet Core
...
- CLI `-addrinfo` now returns a single field for the number of `onion` addresses
  ...
  for Tor V2 addresses was removed from Bitnet Core in 22.0. (#22544)
