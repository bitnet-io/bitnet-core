...
Bitnet Core now supports loading multiple, separate wallets (See [PR 8694](https://github.com/bitnet/bitnet/pull/8694), [PR 10849](https://github.com/bitnet/bitnet/pull/10849)). The wallets are completely separated, with individual balances, keys and received transactions.
...
Bitnet Core has supported creating opt-in replace-by-fee (RBF) transactions
...
In previous versions of Bitnet Core, a portion of each block could be reserved for transactions based on the age and value of UTXOs they spent. This concept (Coin Age Priority) is a policy choice by miners, and there are no consensus rules around the inclusion of Coin Age Priority transactions in blocks. In practice, only a few miners continue to use Coin Age Priority for transaction selection in blocks. Bitnet Core 0.15 removes all remaining support for Coin Age Priority (See [PR 9602](https://github.com/bitnet/bitnet/pull/9602)). This has the following implications:
...
- The second argument in `prioritisetransaction` has been renamed from `priority_delta` to `dummy` since Bitnet Core no longer has a concept of coin age priority. The `dummy` argument has no functional effect, but is retained for positional argument compatibility. See [Removal of Coin Age Priority](#removal-of-coin-age-priority).
...
- #10359 `329fc1d` [tests] functional tests should call BitnetTestFramework start/stop node methods (jnewbery)
...
- #10556 `65cc7aa` Move stop/start functions from utils.py into BitnetTestFramework (jnewbery)
...
- #10958 `659c096` Update to latest Bitnet patches for LevelDB (sipa)
