...
Bitnet Core version 24.0.1 is now available from:
...
installer (on Windows) or just copy over `/Applications/Bitnet-Qt` (on macOS)
...
using the Linux kernel, macOS 10.15+, and Windows 7 and newer.  Bitnet
...
This version of Bitnet Core adds a new `mempoolfullrbf` configuration
...
Some Bitnet services today expect that the first version of an
...
The Bitnet Protocol does not, and cannot, provide any assurance that
...
fees, was realized in [Bitnet Core 0.12.0][] (February 2016) with the
...
default for years, and several contributors to Bitnet Core are
...
[Bitnet Core 0.12.0]: https://bitnetcore.org/en/releases/0.12.0/#opt-in-replace-by-fee-transactions
...
- To help prevent fingerprinting transactions created by the Bitnet Core wallet, change output
