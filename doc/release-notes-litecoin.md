Bitnet Core version 0.18.1 is now available from:

  <https://download.bitnet.org/bitnet-0.18.1/>
...
Please report bugs using the issue tracker at GitHub:
  ...
  <https://github.com/bitnet-project/bitnet/issues>

To receive security and update notifications, please subscribe to:

  <https://groups.google.com/forum/#!forum/bitnet-dev>
...
`/Applications/Bitnet-Qt` (on Mac) or `bitnetd`/`bitnet-qt` (on
Linux).
...
recommended to use Bitnet Core on unsupported systems.
...
10.10. Additionally, Bitnet Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/bitnet-project/bitnet/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@bitnet.org) the Bitnet Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 BIT),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in BIT per kilobyte, not litoshi per byte.

Documentation
-------------

- A new short [document](https://github.com/bitnet-project/bitnet/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/bitnet-project/bitnet/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/bitnet-project/bitnet/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/bitnet-project/bitnet/blob/master/doc/bitnet-conf.md)
  about the `bitnet.conf` file describes how to use it to configure
  Bitnet Core.

- A new document introduces Bitnet Core's BIP174 [Partially-Signed
  Bitnet Transactions
  (PSBT)](https://github.com/bitnet-project/bitnet/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/bitnet-project/bitnet/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Bitnet-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Bitnet Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/bitnet-project/bitnet/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/bitnet-project/bitnet/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Bitnet Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Bitnet Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/bitnet-project/bitnet/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/bitnet-project/bitnet/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- A new `bitnet-wallet` tool is now distributed alongside Bitnet
...
This section describes planned changes to Bitnet Core that may affect
other Bitnet software and services.

- Since version 0.16.0, Bitnet Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Bitnet Core 0.20 (expected about
  a year after 0.18), Bitnet Core will default to native segwit
  ...
  to bech32 addresses, and if the Bitnet Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Bitnet Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Bitnet Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Bitnet Core
...
- A sub-project of Bitnet Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Bitnet Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Bitnet Core's own implementation, although entropy
  gathered by Bitnet Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Bitnet Core a
...
- On macOS, Bitnet Core now opts out of application CPU throttling
