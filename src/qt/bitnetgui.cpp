...
const std::string BitnetGUI::DEFAULT_UIPLATFORM =
...
BitnetGUI::BitnetGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &BitnetGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &BitnetGUI::showModalOverlay);
...
void BitnetGUI::createActions()
{
    ...
    connect(overviewAction, &QAction::triggered, this, &BitnetGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &BitnetGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &BitnetGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    connect(quitAction, &QAction::triggered, this, &BitnetGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &BitnetGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &BitnetGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &BitnetGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &BitnetGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &BitnetGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &BitnetGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &BitnetGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &BitnetGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &BitnetGUI::setPrivacy);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &BitnetGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &BitnetGUI::showDebugWindow);
}

void BitnetGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void BitnetGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &BitnetGUI::setCurrentWalletBySelectorIndex);
...
void BitnetGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &BitnetGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &BitnetGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &BitnetGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &BitnetGUI::showProgress);
...
#ifdef ENABLE_WALLET
void BitnetGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &BitnetGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &BitnetGUI::removeWallet);

    auto activity = new LoadWalletsActivity(m_wallet_controller, this);
    activity->load();
}

WalletController* BitnetGUI::getWalletController()
{
    return m_wallet_controller;
}

void BitnetGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &BitnetGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &BitnetGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &BitnetGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &BitnetGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &BitnetGUI::incomingTransaction);
    connect(this, &BitnetGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
    wallet_view->setPrivacy(isPrivacyModeActivated());
    const QString display_name = walletModel->getDisplayName();
    m_wallet_selector->addItem(display_name, QVariant::fromValue(walletModel));
}

void BitnetGUI::removeWallet(WalletModel* walletModel)
{
    if (!walletFrame) return;
...
void BitnetGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame) return;
...
void BitnetGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void BitnetGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void BitnetGUI::setWalletActionsEnabled(bool enabled)
{
...
void BitnetGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &BitnetGUI::toggleHidden);
...
void BitnetGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void BitnetGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void BitnetGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void BitnetGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void BitnetGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void BitnetGUI::openClicked()
{
...
void BitnetGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void BitnetGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void BitnetGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void BitnetGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void BitnetGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void BitnetGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void BitnetGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void BitnetGUI::updateNetworkState()
{
...
void BitnetGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void BitnetGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void BitnetGUI::updateHeadersSyncProgressLabel()
{
...
void BitnetGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &BitnetGUI::quitRequested);
...
void BitnetGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void BitnetGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void BitnetGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &BitnetGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &BitnetGUI::show);
                e->ignore();
            }
        }
    }
...
void BitnetGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void BitnetGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void BitnetGUI::incomingTransaction(const QString& date, BitnetUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(BitnetUnits::formatWithUnit(unit, amount, true));
...
void BitnetGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void BitnetGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool BitnetGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool BitnetGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void BitnetGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void BitnetGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void BitnetGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void BitnetGUI::updateProxyIcon()
{
    std::string ip_port;
...
void BitnetGUI::updateWindowTitle()
{
...
void BitnetGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void BitnetGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void BitnetGUI::detectShutdown()
{
...
void BitnetGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void BitnetGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(BitnetGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = BitnetGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void BitnetGUI::subscribeToCoreSignals()
{
...
void BitnetGUI::unsubscribeFromCoreSignals()
{
...
bool BitnetGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<BitnetUnit> units = BitnetUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const BitnetUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, BitnetUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const BitnetUnit u : BitnetUnits::availableUnits()) {
        menu->addAction(BitnetUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(BitnetUnit newUnits)
{
    setText(BitnetUnits::longName(newUnits));
}
