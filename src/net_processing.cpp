...
bool ProcessMessages(CNode* pfrom, CConnman& connman, const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (!pfrom->vRecvGetData.empty()) return true;
        ...
        if (memcmp(msg.hdr.pchMessageStart, chainparams.MessageStart(), CMessageHeader::MESSAGE_START_SIZE) != 0) {
            LogPrintf("PROCESSMESSAGE: INSTALL AT LEAST radioCOIN 6.0.1 AuxPow enabled blocks height 100~ INVALID MESSAGESTART %s peer=%d\n", SanitizeString(msg.hdr.GetCommand()), pfrom->id);
            pfrom->fDisconnect = true;
            return false;
        }
        ...
        {
            LogPrintf("PROCESSMESSAGE: INSTALL AT LEAST radioCOIN 6.0.1 AuxPow enabled blocks height 100~ ERRORS IN HEADER %s peer=%d\n", SanitizeString(hdr.GetCommand()), pfrom->id);
            return fMoreWork;
        }
