...
#if defined(NDEBUG)
# error "Bitnet cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Bitnet Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetBitnetMinRelayFee(tx, nSize, !fLimitFree))
        {
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    if (nHeight == 1) return COIN * 10000;
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Bitnet: Switch maturity at depth 145,000
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Bitnet: BIP30 has been active since inception
    ...
    // Bitnet: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetBitnetBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Bitnet: Disallow AuxPow blocks before it is activated.
    ...
    // Bitnet: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Bitnet: We probably want to disable this
    int nLockTimeFlags = 0;
