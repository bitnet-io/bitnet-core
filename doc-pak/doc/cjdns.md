# CJDNS support in Bitnet Core

It is possible to run Bitnet Core over CJDNS, an encrypted IPv6 network that
...
done outside of Bitnet Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Bitnet network and individual nodes.
...
before it will work with your Bitnet Core node. This is described in steps
...
## Run Bitnet Core with CJDNS

Once you are connected to the CJDNS network, the following Bitnet Core
...
When enabled, this option tells Bitnet Core that it is running in an
...
IPv6 local network. This helps Bitnet Core perform better address management:
...
CJDNS support was added to Bitnet Core in version 23.0 and there may be fewer
...
## CJDNS-related information in Bitnet Core

There are several ways to see your CJDNS address in Bitnet Core:
