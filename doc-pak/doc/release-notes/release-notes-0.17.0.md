Bitnet Core version 0.17.0 is now available from:
...
BIP 174 Partially Signed Bitnet Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitnet/bips/blob/master/bip-0174.mediawiki) is an interchange format for Bitnet transactions that are not fully signed
...
Overall, the construction of a fully signed Bitnet transaction goes through the
following steps:
...
- An **Extractor** produces a valid Bitnet transaction (in network format)
...
- #13528 `b877c39` Move BitnetGUI initializers to class, fix initializer order warning (laanwj)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use BitnetTestFramework (jnewbery)
...
- #11772 `0630974` Change invalidblockrequest to use BitnetTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use BitnetTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use BitnetTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for BitnetTestFramework (WillAyd)
