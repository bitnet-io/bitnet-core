Bitnet Core version 1.14.3 is now available from:

  <https://github.com/bitnet/bitnet/releases/tag/v1.14.3/>
...
To receive security and update notifications, please watch reddit or Twitter:
  ...
  * @Bitnet on Twitter for high priority announcements
  * @bitnet\_devs on Twitter for updates on development work
...
Bitnet Core is extensively tested on Ubuntu Server LTS, Mac OS X and Windows 10.
...
Block download timeouts are expressed as a multiple of block interval, and as such Bitnet block download times were relatively aggressive, leading to a high number of timeouts. Increased the timeouts to be more flexible to real world conditions.
...
* Return unspents greater or equal than a specific amount in RADC: minimumAmount (default = 0).
* Return unspents lower or equal than a specific amount in RADC: maximumAmount (default=unlimited).
* Return unspents with a total number lower or equal than a specific number: maximumCount (default=0=unlimited).
* Return unspents which total is greater or equal than a specific amount in RADC: minimumSumAmount (default=unlimited).
...
* Change IPC prefix from `bitcoin:` to `bitnet:`.
