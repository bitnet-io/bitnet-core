...
class QtumBlockHeaderTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        """
        tx_hex = node.createcontract("60606040523415600b57fe5b5b60398060196000396000f30060606040525b600b5b5b565b0000a165627a7a72305820693c4900c412f72a51f8c01a36d38d9038d822d953faf5a5b28e40ec6e1a25020029", 1000000, BIT_MIN_GAS_PRICE_STR, spendable_addresses.pop(-1), False)['raw transaction']
        ...
        tx_hex = node.sendtocontract(contract_address, "00", 1, 100000, BIT_MIN_GAS_PRICE_STR, spendable_addresses.pop(-1), False)['raw transaction']
        f = io.BytesIO(hex_str_to_bytes(tx_hex))
        tx = CTransaction()
        tx.deserialize(f)
