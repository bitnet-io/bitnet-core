...
class SendToContractTest(BitcoinTestFramework):
    ...
    def sendtocontract_specify_sender_test(self):
        ...
        ret = self.node.sendtocontract(self.contract_address, "2bcf51b4", 100, 1000000, BIT_MIN_GAS_PRICE_STR, "qabmqZk3re5b9UpUcznxDkCnCsnKdmPktT")
        assert('txid' in ret)
        assert('sender' in ret)
        assert('hash160' in ret)
        time.sleep(0.1)
    ...
    def sendtocontract_no_broadcast(self):
        ...
        self.node.generate(1)
        # call setSenderAsOwner with 100 qtum
        ret = self.node.sendtocontract(self.contract_address, "2bcf51b4", 100, 1000000, BIT_MIN_GAS_PRICE_STR, "qabmqZk3re5b9UpUcznxDkCnCsnKdmPktT", False)
